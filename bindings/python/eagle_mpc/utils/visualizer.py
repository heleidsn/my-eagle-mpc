...
class MulticopterMpcDisplay(crocoddyl.GepettoDisplay):
    def __init__(self,
                 ...
                 payload='',
                 cog=False):
        crocoddyl.GepettoDisplay.__init__(
            self, robot, rate, freq, cameraTF, floor, frameNames, visibility)

        self.baseParams = baseParams
        self.thrustGroup = "world/" + self.robotName + "/thrusts"
        self.thrusts = ["rotor" + str(i)
                        for i in range(self.baseParams.n_rotors)]
    ...
    def display(self, xs, us=[], fs=[], ps=[], se3s=[], dts=[], payloads=[], cogs=[], factor=1.):
        ...
        for i, x in enumerate(xs):
            if not i % S:
                if fs:
                    self.activeContacts = {k: False for k,
                                           c in self.activeContacts.items()}
                    for f in fs[i]:
                        key = f["key"]
                        pose = f["oMf"]
                        wrench = f["f"]
                ...
                if se3s:
                    for key, se3 in se3s[i].items():
                        self.robot.viewer.gui.applyConfiguration(
                            self.frameAxisGroup + "/" + str(key), se3)

                if (self.payload == 'box' or self.payload == 'sphere') and payloads:
                    self.robot.viewer.gui.applyConfiguration(
                        self.payloadGroup, payloads[i])

                if cogs and self.cog:
                    self.robot.viewer.gui.applyConfiguration(
                        self.cogGroup + "/ball", cogs[i])
            ...
            if hasattr(data, "differential"):
                if isinstance(data.differential,
                    ...
                    for key, contact in data.differential.multibody.contacts.contacts.todict().items():
                        if model.differential.contacts.contacts[key].active:
                            ...
                            fiMo = pinocchio.SE3(
                                contact.pinocchio.oMi[joint].rotation.T, contact.jMf.translation)
                            force = fiMo.actInv(contact.f)
                            R = np.eye(3)
                            mu = 0.7
                            for k, c in model.differential.costs.costs.todict().items():
                                if isinstance(c.cost, crocoddyl.libcrocoddyl_pywrap.ResidualModelContactFrictionCone):
                                    if contact.frame == c.cost.reference.id:
                                        R = c.cost.reference.cone.R
                                        mu = c.cost.reference.cone.mu
                                        continue
                            fc.append({"key": str(joint), "oMf": oMf,
                                      "f": force, "R": R, "mu": mu})
                    fs.append(fc)
                    ...
                    if isinstance(data.differential[0],
                        ...
                        for key, contact in data.differential[0].multibody.contacts.contacts.todict().items():
                            if model.differential.contacts.contacts[key].active:
                                joint = model.differential.state.pinocchio.frames[contact.frame].parent
                                oMf = contact.pinocchio.oMi[joint] * \
                                    contact.jMf
                                fiMo = pinocchio.SE3(
                                    contact.pinocchio.oMi[joint].rotation.T, contact.jMf.translation)
                                force = fiMo.actInv(contact.f)
                                ...
                                for k, c in model.differential.costs.costs.todict().items():
                                    if isinstance(c.cost, crocoddyl.libcrocoddyl_pywrap.ResidualModelContactFrictionCone):
                                        if contact.frame == c.cost.reference.id:
                                            ...
                                            continue
                                fc.append(
                                    {"key": str(joint), "oMf": oMf, "f": force, "R": R, "mu": mu})
                        fs.append(fc)
                ...
                for key, impulse in data.multibody.impulses.impulses.todict().items():
                    if model.impulses.impulses[key].active:
                        oMf = impulse.pinocchio.oMi[impulse.joint] * \
                            impulse.jMf
                        fiMo = pinocchio.SE3(
                            impulse.pinocchio.oMi[impulse.joint].rotation.T, impulse.jMf.translation)
                        ...
                        for k, c in model.costs.costs.todict().items():
                            if isinstance(c.cost, crocoddyl.libcrocoddyl_pywrap.ResidualModelContactFrictionCone):
                                if impulse.joint == self.robot.model.frames[c.cost.id].parent:
                                    ...
                                    mu = c.cost.cone.mu
                                    continue
                        fc.append({"key": str(impulse.joint),
                                  "oMf": oMf, "f": force, "R": R, "mu": mu})
                fs.append(fc)
        return fs
                ...
                if hasattr(dataDiff, "pinocchio"):
                    ...
                    for frameId in self.frameAxisNames:
                        pinocchio.updateFramePlacement(
                            model.differential.pinocchio, dataDiff.pinocchio, int(frameId))
        ...
        for i, data in enumerate(datas):
            ...
            if hasattr(data, "differential"):
                if isinstance(data.differential, crocoddyl.libcrocoddyl_pywrap.StdVec_DiffActionData):
                    ...
                    dataDiff = data.differential
                if hasattr(dataDiff, "pinocchio"):
                    cog = pinocchio.centerOfMass(
                        model.differential.pinocchio, data.differential.pinocchio, q, False)
    ...
    def _addThrustArrows(self):
        ...
        if self.fullVisibility:
            self.robot.viewer.gui.setVisibility(
                self.thrustGroup, "ALWAYS_ON_TOP")
    ...
    def _addPayload(self, type):
        if type == 'box':
            ...
            self.robot.viewer.gui.addSphere(
                self.payloadGroup, self.payloadSphereSize, self.payloadColor)

    def _addFrameAxis(self):
        for frame in self.frameAxisNames:
            self.robot.viewer.gui.addXYZaxis(
                self.frameAxisGroup + "/" + frame, [1., 0., 0., 1.], .01, 0.1)

    def _addCog(self):
        self.robot.viewer.gui.addSphere(
            self.cogGroup + "/ball", self.cogRadius, self.cogColor)
